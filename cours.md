...
Lancer le serveur avec nodemon permet de recharger automatique le serveur à chaque changement dans vos fichiers, un peu comme fait l'extension Live Server.
...
Un middleware est une fonction qui à accès à `req`, `res`, et à `next()`. Elle s'exécute **avant** que la requête n'atteigne sa route finale, et peut :
...
Les middlewares s'exécutent **dans l'ordre où ils sont déclarés** avec `app.use()`.
...
app.use(express.urlencoded({extended: true}))   // parse les formulaires classiques
